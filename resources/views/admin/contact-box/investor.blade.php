...
<style>
  ...
  #dt-investor-inquiries input[type=checkbox] {
    width: 20px;
    height: 20px;
  }
</style>
      ...
      <div class="card">
        <div class="card-body">
          <form method="POST" action="{{ url('admin/contact-box/read-inquiries') }}">
            @csrf
            <table id="dt-investor-inquiries" class="table table-bordered dt-responsive" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
              <thead>
                <tr>
                  <th>
...
<script>
  $(document).ready(function() {
    var table = $("#dt-investor-inquiries").DataTable({
      ajax: "{{ url('admin/contact-box/get-inquiries') }}/Investor",
      columns: [{
          data: "id"
        },
        {
    ...
    $('.del-btn').html(
      '<button type="button" class="btn btn-sm btn-danger waves-effect waves-light del-review" id="delete-selected"><i class="fas fa-trash-alt me-1"></i>Delete Selected</button><button type="submit" class="ms-2 btn btn-sm btn-primary waves-effect waves-light"><i class="fas fa-envelope-open-text"></i> Mark as Read</button>'
    );
