                    ...
                    <div class="row">
                        ...
                        <div class="mb-3 mt-1 col-md-6">
                            <div class="justify-content-end d-flex">
                                <a href="{{ url('admin/user-record/post-delivery') }}" class="btn btn-primary btn-sm"> 돌아가기</a>
                            </div>
                        </div>
                    </div>
                    <hr class="my-auto flex-grow-1 mt-1 mb-3" style="height:1px;">
                    <form class="form-horizontal mt-4 needs-validation" method="POST" action="{{ url('admin/user-record/edit_post_delivery/'.$postdelivery->id) }}" novalidate>
                        @csrf
                        <div class="row">
                            <div class="col-md-4">
                                <div class="mb-3">
                                    <label class="form-label" for="date">날짜 <span class="text-danger">*</span></label>
                                    <input type="text" class="form-control" id="date" name="date" value="{{date('Y.m.d', strtotime($postdelivery->date))}}" required>
                                </div>
                            </div>
                            <div class="col-md-4">
                                    ...
                                    <div class="input-group">
                                        <select class="form-control select2" id="company" name="company" required>
                                            ...
                                            @foreach($companies as $company)
                                                <option value="{{$company->id}}" @if($company->id == $postdelivery->companyId) selected @endif>{{$company->companyName}}</option>
                                            @endforeach
                                        </select>
                            ...
                            <div class="col-md-4">
                                <div class="mb-3">
                                    <label class="form-label" for="city">지역 <span class="text-danger">*</span></label>
                                    <select class="form-control select2" id="city" name="city" required>
                                        <option value="">지역..</option>
                                        @foreach($cities as $city)
                                            <option value="{{$city->id}}" @if($city->id == $postdelivery->cityId) selected @endif>{{$city->cityName}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="col-md-4">
                                <div class="mb-3">
                                    ...
                                    <select class="form-control select2" id="post_status" name="post_status" required>
                                        ...
                                        <option value="Delivered" @if($postdelivery->status == "Delivered") selected @endif>발송완료</option>
                                        <option value="Pending" @if($postdelivery->status == "Pending") selected @endif>발송중</option>
                                        <option value="Returned" @if($postdelivery->status == "Returned") selected @endif>반송됨</option>
                                        <option value="Canceled" @if($postdelivery->status == "Canceled") selected @endif>취소</option>
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="mb-3">
                                    ...
                                    <select class="form-control select2" id="customer" name="customer" required>
                                        ...
                                        @foreach($customers as $customer)
                                            <option value="{{$customer->id}}" @if($customer->id == $postdelivery->userId) selected @endif>{{$customer->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                <div class="mb-3">
                                    ...
                                    <input type="text" value="{{ $postdelivery->address }}" class="form-control pastAddress" id="upostaddress" name="upostaddress" required>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="mt-4 pt-1">
                                    <input class="btn btn-info w-100 fw-bold copyAddress" data-address="{{ $postdelivery->address }}" type="button" value="기존 주소 복사">
                                </div>
                            </div>
                        </div>
