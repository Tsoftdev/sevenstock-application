...
@section('title', 'News Room Video Recommend')
@section('content')
            ...
            <div class="col-md-6">
              <div class="rec-box">
                <a href="#" data-bs-toggle="modal" data-bs-target="#recommendSelection" data-box-id="1" id="rec-box-1">
                  <div class="border p-2 mb-3 text-center d-flex" style="height: 288px; justify-content: center;
                  align-items: center;">
                    @if ($news1)
                    <div class="rec-model">
                      <span class="tag-keyword">{{ $news1->video->tag->keyword }}</span>
                      <h6>{{ \Illuminate\Support\Str::limit($news1->video->title, 32, $end='...') }}</h6>
                      <span class="date">{{ $news1->video->date }}</span>
                    </div>
                    @else
                ...
                @if ($news1)
                <form method="POST" action="{{ url('admin/change-content/recommend/video-unrecommend/' . $news1->id) }}">
                  @csrf
              ...
              <div class="rec-box">
                <a href="#" data-bs-toggle="modal" data-bs-target="#recommendSelection" data-box-id="2" id="rec-box-2">
                  ...
                  align-items: center;">
                    @if ($news2)
                    <div class="rec-model">
                      <span class="tag-keyword">{{ $news2->video->tag->keyword }}</span>
                      <h6>{{ \Illuminate\Support\Str::limit($news2->video->title, 32, $end='...') }}</h6>
                      <span class="date">{{ $news2->video->date }}</span>
                    </div>
                    @else
                    <i class="fas fa-plus-circle fs-4"></i>
                    @endif
                ...
                @if ($news2)
                <form method="POST" action="{{ url('admin/change-content/recommend/video-unrecommend/' . $news2->id) }}">
                  @csrf
                  <button type="submit" class="btn btn-sm btn-danger del-rec"><i class="fas fa-trash-alt"></i></button>
                </form>
                @endif
              ...
              <div class="rec-box">
                <a href="#" class="rec-box" data-bs-toggle="modal" data-bs-target="#recommendSelection" data-box-id="3" id="rec-box-3">
                  ...
                  align-items: center;">
                    @if ($news3)
                    <div class="rec-model">
                      <span class="tag-keyword">{{ $news3->video->tag->keyword }}</span>
                      <h6>{{ \Illuminate\Support\Str::limit($news3->video->title, 32, $end='...') }}</h6>
                      <span class="date">{{ $news3->video->date }}</span>
                    </div>
                    @else
                    <i class="fas fa-plus-circle fs-4"></i>
                    @endif
                ...
                @if ($news3)
                <form method="POST" action="{{ url('admin/change-content/recommend/video-unrecommend/' . $news3->id) }}">
                  @csrf
                  <button type="submit" class="btn btn-sm btn-danger del-rec"><i class="fas fa-trash-alt"></i></button>
                </form>
                @endif
              ...
              <div class="rec-box">
                <a href="#" class="rec-box" data-bs-toggle="modal" data-bs-target="#recommendSelection" data-box-id="4" id="rec-box-4">
                  ...
                  align-items: center;">
                    @if ($news4)
                    <div class="rec-model">
                      <span class="tag-keyword">{{ $news4->video->tag->keyword }}</span>
                      <h6>{{ \Illuminate\Support\Str::limit($news4->video->title, 32, $end='...') }}</h6>
                      <span class="date">{{ $news4->video->date }}</span>
                    </div>
                    @else
                    <i class="fas fa-plus-circle fs-4"></i>
                    @endif
                ...
                @if ($news4)
                <form method="POST" action="{{ url('admin/change-content/recommend/video-unrecommend/' . $news4->id) }}">
                  @csrf
                  <button type="submit" class="btn btn-sm btn-danger del-rec"><i class="fas fa-trash-alt"></i></button>
                </form>
                @endif
              ...
              <div class="rec-box">
                <a href="#" class="rec-box" data-bs-toggle="modal" data-bs-target="#recommendSelection" data-box-id="4" id="rec-box-4">
                  ...
                  align-items: center;">
                    @if ($news5)
                    <div class="rec-model">
                      <span class="tag-keyword">{{ $news5->video->tag->keyword }}</span>
                      <h6>{{ \Illuminate\Support\Str::limit($news5->video->title, 32, $end='...') }}</h6>
                      <span class="date">{{ $news5->video->date }}</span>
                    </div>
                    @else
                    <i class="fas fa-plus-circle fs-4"></i>
                    @endif
                ...
                @if ($news5)
                <form method="POST" action="{{ url('admin/change-content/recommend/video-unrecommend/' . $news5->id) }}">
                  @csrf
                  <button type="submit" class="btn btn-sm btn-danger del-rec"><i class="fas fa-trash-alt"></i></button>
                </form>
                @endif
            ...
            <div class="col-md-12">
              <table id="dt-news-random" class="table dt-responsive" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                ...
                <tbody>
                  ...
                  <tr>
                    <td width="150"><img src="{{$rand->video->images ? $rand->video->images[0] : ''}}" class="img-fluid" alt=""></td>
                    <td>
                      <div class="row">
                        <div class="col-md-8">
                          <h5 class="mb-0">{{$rand->video->title}}</h5>
                          <span class="badge bg-dark">{{$rand->video->date}}</span>
                          @if ($rand->video->tag_id != 0)
                          <span class="badge text-uppercase" style="background-color: {{$rand->video->tag->color}}">{{$rand->video->tag->keyword}}</span>
                          @else
                          <strong><small>(Untagged)</small></strong>
                          @endif
                        </div>
                        <div class="col-md-4 text-end">
                          <form method="POST" action="{{ url('admin/change-content/recommend/del-video-random') . '/' . $rand->id }}">
                            @csrf
                        ...
                        <div class="col-md-12">
                          {!!\Illuminate\Support\Str::limit(strip_tags($rand->video->description), 100, $end='...')!!}
                        </div>
                      </div>
                    </td>
                  </tr>
...
<script>
  ...
  $(function() {
    ...
    $('body').on('click', '.rec-box a', function() {
      ...
      $('.news-list-form').attr('action', '<?php echo url('admin/change-content/recommend/video-recommend'); ?>');
    ...
    $('body').on('click', '.add-random', function() {
      $('.news-list-form').attr('action', '<?php echo url('admin/change-content/recommend/add-video-random'); ?>');
    });
