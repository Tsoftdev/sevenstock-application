    ...
    <div class="col-md-6">
      <div class="card">
        <div class="card-body">
          ...
          <div class="row mb-3">
            <div class="col-md-6">
              <h4 class="card-title">Blog Top Posts</h4>
            </div>
            ...
            <div class="col-md-12">
              <table id="dt-news-random" class="table dt-responsive" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                ...
                <tbody>
                  ...
                  <tr>
                    <td width="150"><img src="{{$rand->blog->images[0]}}" class="img-fluid" alt=""></td>
                    <td>
                      <div class="row">
                        <div class="col-md-8">
                          <h5 class="mb-0">{{$rand->blog->title}}</h5>
                          <span class="badge bg-dark">{{$rand->blog->date}}</span>
                          @if ($rand->blog->tag_id != 0)
                          <span class="badge text-uppercase" style="background-color: {{$rand->blog->tag->color}}">{{$rand->blog->tag->keyword}}</span>
                          @else
                          <strong><small>(Untagged)</small></strong>
                          @endif
                        </div>
                        <div class="col-md-4 text-end">
                          <form method="POST" action="{{ url('admin/change-content/recommend/del-blog-recommend') . '/' . $rand->id }}">
                            @csrf
                        ...
                        <div class="col-md-12">
                          {!!\Illuminate\Support\Str::limit(strip_tags($rand->blog->description), 100, $end='...')!!}
                        </div>
                      </div>
                    </td>
                  </tr>
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">Select Blog Post</h5>
...
<script>
  ...
  $(function() {
    ...
    $('body').on('click', '.add-random', function() {
      $('.news-list-form').attr('action', '<?php echo url('admin/change-content/recommend/add-blog-recommend'); ?>');
    });
