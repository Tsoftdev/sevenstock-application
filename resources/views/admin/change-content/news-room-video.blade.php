...
@section('title', 'video Room Video')
@section('css')
<!-- Sweet Alert-->
...
<div class="container-fluid">
  ...
  <div class="row">
    <div class="col-md-6">
      <div class="card">
        <div class="card-body">
          <form id="video-form" class="needs-validation" enctype="multipart/form-data" novalidate data-id="">
            <div class="row mb-3 gx-3">
              <div class="col-md-6">
                <div class="row gx-1">
                  <div class="col-md-8">
                    <select class="form-select video-tags" name="tag" required>
            ...
            <div class="row mb-3">
              <div class="col-md-12">
                <input class="form-control" name="video_link" type="text" value="" id="video-link" placeholder="Video Link" required>
              </div>
            </div>
            <div class="row">
              <input type="file" class="imagepicker" name="video_images[]" />
            </div>
            <hr>
            <div class="row">
              <div class="col-md-12 text-end">
                <button type="submit" id="add-video-btn" class="btn btn-primary waves-effect waves-light">Save</button>
                <button type="submit" id="update-video-btn" class="btn btn-warning waves-effect waves-light d-none">Update</button>
                <a href="#" id="cancel-video-btn" class="btn btn-danger waves-effect waves-light d-none">Cancel</a>
              </div>
            </div>
          </form>
        </div>
    ...
    <div class="col-md-6">
      ...
      <div class="card">
        <div class="card-body">
          <!-- Select Tags -->
          <div class="mb-4">
            <select class="form-control select2 video-tags" id="tag-filter">
...
<div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasRight" aria-labelledby="offcanvasRightLabel" style="width: 300px">
  ...
  <div class="offcanvas-body pt-0">
    ...
    <div class="row">
      <div class="col-md-12">
        <table id="dt-video-tags" class="table dt-responsive" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
          <thead>
            <tr>
              <th>Keyword</th>
              <th></th>
...
<script>
  $(document).ready(function() {
    ...
    // video form buttons
    const savevideoBtn = $('#add-video-btn');
    const updatevideoBtn = $('#update-video-btn');
    const cancelvideoBtn = $('#cancel-video-btn');
    ...
    0 < $("#description").length && tinymce.init({
      ...
      images_upload_url: '/admin/video-upload',
    ...
    // Tags Datatable
    var tableTags = $("#dt-video-tags").DataTable({
      dom: "<'search-bar' f>rt",
      paging: false,
      ajax: "{{ url('admin/video-tags') }}",
      language: {
        search: "",
      ...
      $.ajax({
        ...
        success: function(response) {
          ...
          if (response.status == 'ok') {
            ...
            $('.video-tags').append(`<option value="${response.data.id}">${response.data.keyword}</option>`)
          } else {
            // Loader
            $('.tag-loader.fa-plus').removeClass('d-none');
            $('.tag-loader.fa-spinner').addClass('d-none');
    ...
    $('body').on('click', '.del-tag', function() {
      ...
      Swal.fire({
        title: 'Are you sure?',
        text: "You won't be able to revert this and all video posts related to this tag will be untagged.",
        icon: 'warning',
        showCancelButton: !0,
          ...
          $.ajax({
            ...
            success: function(response) {
              ...
              if (response.status == 'ok') {
                ...
                $(".video-tags option[value='" + id + "']").remove();
              }
            }
          });
      });
          ...
          render: function(data, type, row, meta) {
            ...
            return `<div class="row">
                      ...
                      <div class="col-md-4 text-end">
                        <a href="#" type="button" class="btn btn-sm btn-info waves-effect waves-light edit-video" data-id="${row.id}"><i class="fas fa-edit"></i></a>
                        <button type="button" class="btn btn-sm btn-danger waves-effect waves-light del-video" data-id="${row.id}"><i class="fas fa-trash-alt"></i></button>
                      </div>
                      <div class="col-md-12">
    ...
    // video form submit
    $('body').on('submit', '#video-form', function(e) {
      e.preventDefault();
      ...
      // video ID - If updating video
      const videoID = $(this).data('id');

      // Ajax request
      $.ajax({
        url: '/admin/video' + ((videoID) ? '/' + videoID : ''),
        headers: {
          'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content'),
        },
        type: 'POST',
        ...
        success: function(response) {
          ...
          if (response.status == 'ok') {
            ...
            $('#video-form').removeClass('was-validated');
            ...
            if (videoID) {
              ...
              updatevideoBtn.addClass('d-none');
              cancelvideoBtn.addClass('d-none');
            ...
            if (response.data.video_images) {
              errors += `<li>${response.data.video_images[0]}</li>`;
            }
    ...
    $('body').on('click', '.edit-video', function() {
      ...
      updatevideoBtn.removeClass('d-none');
      cancelvideoBtn.removeClass('d-none');

      // Fill in video id
      $('#video-form').data('id', id);
      ...
      $.ajax({
        ...
        success: function(response) {
          ...
          if (response.status == 'ok') {
            $('.video-tags option[value="' + video.tag_id + '"]').prop('selected', true);
            $('#date').val(video.date);
            $('#title').val(video.title);
            $('#video-link').val(video.video_link);
            tinyMCE.activeEditor.setContent(video.description);
